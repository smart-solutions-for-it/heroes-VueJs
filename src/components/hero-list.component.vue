<template>
    <div id="heroList">
        ...
        <h4>My heroes</h4>
        <ul class='list-group'>
            <li class="list-group-item link" v-for="hero in heroes">
...
<script>
    ...
    export default {
        ...
        methods: {
            addHero() {
                this.service.methods.addHero(this.newHero).then(response => this.heroes.push(response.body),
                    error => this.errorMessage = error.statusText);
            },
            deleteHero(heroId) {
                this.service.methods.deleteHero(heroId).then(response => this.heroes.splice(heroId,1),
                    error => this.errorMessage = error.statusText);
            }
